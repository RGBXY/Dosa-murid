    ...
    <div class="mt-4 bg-white rounded-xl p-4 container mx-auto">
        ...
        <form action="{{route('pelanggaran.store.aksi')}}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="date" id="tanggal" name="tanggal" class="bg-gray-50 border border-gray-300 text-lg rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
            </div>


            <div class="mb-4">
                <label for="waktu" class="block mb-2 text-lg font-medium">Waktu</label>
                <input type="time" id="waktu" name="waktu" class="bg-gray-50 border border-gray-300 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
            </div>

            <div class="mb-4">
                <label for="kode_bk" class="block mb-2 text-lg font-medium">Guru BK</label>
                <select id="kode_bk" name="kode_bk" class="bg-gray-50 border border-gray-300  text-lg rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                    <option value="" disabled>--PILIH GURU BK--</option>

                    @foreach($guruBK as $key => $bk)
                    <option value="{{$bk->kode_bk}}">{{$bk->nama}}</option>
                    @endforeach
                </select>
            </div>

            <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-md px-5 py-2.5 text-center ">Catat
                Pelanggaran</button>
        </form>
        @endif
    </div>
